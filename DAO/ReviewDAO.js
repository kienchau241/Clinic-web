...
exports.getReview = async function (id) {
  ...
  let result = await dbConfig.db.pool
    ...
    .query(
      `SELECT * from ${ReviewSchema.schemaName} where ${ReviewSchema.schema.id.name} = @${ReviewSchema.schema.id.name}`
    );
...
exports.getReviewByTourAndUser = async function (TreatmentId, userId) {
  ...
  let result = await dbConfig.db.pool
    .request()
    .input(
      ReviewSchema.schema.TreatmentId.name,
      ReviewSchema.schema.TreatmentId.sqlType,
      TreatmentId
    )
    .input(
      ReviewSchema.schema.userId.name,
      ReviewSchema.schema.userId.sqlType,
      userId
    )
    .query(
      `SELECT * from ${ReviewSchema.schemaName} where ${ReviewSchema.schema.TreatmentId.name} = @${ReviewSchema.schema.TreatmentId.name} AND ${ReviewSchema.schema.userId.name} = @${ReviewSchema.schema.userId.name}`
    );
...
exports.updateReview = async function (id, updateReview) {
  ...
  request.input(
    ReviewSchema.schema.id.name,
    ReviewSchema.schema.id.sqlType,
    id
  );
  query +=
    " " +
    updateStr +
    ` where ${ReviewSchema.schema.id.name} = @${ReviewSchema.schema.id.name}`;
...
exports.deleteReview = async function (id) {
  ...
  let result = await dbConfig.db.pool
    ...
    .input(ReviewSchema.schema.id.name, ReviewSchema.schema.id.sqlType, id)
    .query(
      `delete ${ReviewSchema.schemaName} where ${ReviewSchema.schema.id.name} = @${ReviewSchema.schema.id.name}`
    );
